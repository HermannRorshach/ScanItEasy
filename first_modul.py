...
def open_editor_window():
    editor_window = GraphicalEditor()  # Создаем экземпляр окна
    editor_window.mainloop()  # Запускаем главный цикл окна

button = ctk.CTkButton(win, text="Открыть графический редактор", command=open_editor_window)
button.pack(pady=20)
...
class GraphicalEditor(ctk.CTk):
    def __init__(self):
        ...
        # Загружаем изображения как атрибуты

        self.configure_grid()  # Должен быть вызван для self, который является CTk
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        if not os.path.exists("dark.png"):
            print("dark.png не найден")
        else:
            print("dark.png найден")
        if not os.path.exists("light.png"):
            print("light.png не найден")
        else:
            print("light.png найден")
