...
class Converter:
    ...
    def docx_to_pdf(self, pages_to_keep=None):
        """Преобразование DOCX в PDF и сохранение только указанных страниц."""
        ...
        if pages_to_keep and pages_to_keep != "all":
            ...
            for page_num in pages_to_keep:
                ...
                if page_num < len(pdf_document):  # Проверяем, что страница существует
            ...
            logging.info(f"Оставлены только указанные страницы. Итоговый файл: {self.pdf_file}")
        else:
            logging.info("Не указаны страницы для удаления. Все страницы сохранены.")


    def pdf_to_png(self, dpi=300, grayscale=True, pages_to_convert=None):
        """Преобразование страниц PDF в PNG и создание PNG-файлов с заданным разрешением.
        Если grayscale=True, конвертирует в чёрно-белые изображения, если False — в цветные.
        """
        ...
        for page_num in pages_to_convert:
            if page_num < doc.page_count:  # Проверяем, что страница существует
                ...
                # Преобразование в чёрно-белый или цветной формат в зависимости от параметра
                if grayscale:
                    pix_bw = pymupdf.Pixmap(pymupdf.csGRAY, pix)  # Чёрно-белое изображение
    ...
    def pngs_to_pdf(self, output_pdf, png_files):
        """Создание нового PDF из PNG-файлов с учётом ориентации и центрированием изображения на странице."""
        A4_WIDTH, A4_HEIGHT = 595, 842  # Ширина и высота страницы A4 в пунктах
        doc = pymupdf.open()

        for png_file in png_files:
            img = pymupdf.open(png_file)
            rect = img[0].rect
            is_landscape = rect.width > rect.height  # Проверка ориентации изображения

            # Устанавливаем ориентацию страницы в зависимости от изображения
            page_width, page_height = (A4_HEIGHT, A4_WIDTH) if is_landscape else (A4_WIDTH, A4_HEIGHT)
            ...
            pdf_page.insert_image(pymupdf.Rect(x_offset, y_offset, x_offset + new_width, y_offset + new_height), filename=png_file)
    ...
    def resource_path(relative_path):
        ...
        logger.debug(f"Базовый путь: {base_path}")  # Заменяем print на логирование
        full_path = os.path.join(base_path, relative_path)
        logger.debug(f"Полный путь к ресурсу: {full_path}")  # Заменяем print на логирование
        return full_path
...
def merge_multiple_pdfs(files, pages, output_path):
    ...
    for i, pdf_file in enumerate(files):
        ...
        if not pages[i]:  # Строка пуста, значит добавляем все страницы
            pages[i] = list(range(pdf_document.page_count))  # Создаем список всех страниц
        print("Список страниц файла", pdf_file, pages[i])

        # Получаем страницы для текущего файла
        for page_num in pages[i]:
            # Загружаем страницу (нумерация с 0, так что page_num уже правильный)
...
def docx_modes(settings):
    ...
    try:
        ...
        mode = settings.get("mode") == "custom_mode"
    ...
    try:
        ...
        if not mode == "compress_mode":
            if mode == "grayscale_mode":
                output_path = "Temp.pdf"
                final_output_path = settings.get("pdf_path").replace(".pdf", "_grayscale.pdf")
                if not is_compression_needed:
                    output_path = final_output_path

            grayscale = mode == "grayscale_mode"
            logging.info("Преобразуем PDF в черно-белый")
            temp_images = converter.pdf_to_png(grayscale=grayscale, pages_to_convert=pages)
            settings["progress"] = 50
        ...
        if mode == "grayscale_mode":
            converter.clean_temp_files()
            logging.info("Временные файлы удалены")
        if mode in ("grayscale_mode", "compress_mode"):
            return final_output_path



def work_process(settings):
    logging.info("Начало программы")
    logging.info(f"settings = {settings}")
    print(f"settings = {settings}")
    is_compression_needed = settings.get("is_compression_needed")
    if settings.get("mode") in ("default_mode", "custom_mode", "convert_to_pdf_mode"):
        ...
        if is_compression_needed:
            ...
            Converter.compress_pdf(input_pdf="merged_output.pdf", output_pdf=final_output_path)
            logging.info("Сжатие завершено")
            if os.path.exists("merged_output.pdf"):
                print('Удаляем файл "merged_output.pdf"')
                os.remove("merged_output.pdf")
    elif settings.get("mode") in ("grayscale_mode", "convert_to_png_mode", "compress_mode"):
